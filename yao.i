...
write,format="\n Yao version %s, Modified by Felix Widmann\n\n",yaoVersion;
...
*/
{
  ...
  if (!is_set(skipReadPhaseScreens)) {
    ...
    // ... and now in microns.
    // write,format="Atmospheric weighting factor is %.3f \n",weight;
    ...
    // Save used CreatePhaseScreens
    fname = swrite(format="%s/%s_pscreens.fits",YAO_SAVEPATH,parprefix);
    yao_fitswrite,fname,float(pscreens);
    
    
  // Write the weight into a file
  if (loop.method == "closed-loop"){
    g = open(YAO_SAVEPATH+parprefix+"_weight","w");
    write,g,weight;
    close,g;
  }
...
*/
{
  ...
  // sphase is the output of _get2dPhase
  
  bphase(_n1:_n2,_n1:_n2) = sphase;
  
  // Write the weight into a file
//  if (loop.method == "closed-loop"){
//    g = open(YAO_SAVEPATH+parprefix+"_sweight","w");
//    write,g,weight;
//    close,g;
//  }

  // Safe phase for evaluation
  // only for first wfs
  // for MCAO/GLAO 1st WFS should be in the middle, TT detection
  if (loop.method == "closed-loop"){
      if (nn == 1){
          if (loopCounter == 1){
             write,format="Saving used phasescreens for WFS number %i \n",nn;
          }
          fname = swrite(format="%s/%s_weight_%04i.fits",YAO_SAVEPATH,parprefix,loopCounter);
          yao_fitswrite,fname,float(sphase),append=1;
      }
    }
...
//----------------------------------------------------
func aoall(parfile,disp=,dpi=,clean=,controlscreen=,keepdmconfig=,savephase=,savedm=,saveslope=)
/* DOCUMENT aoall(parfile,disp=,dpi=,clean=,controlscreen=,keepdmconfig=,savephase=,savedm=,saveslope=)
   Shorthand for aoread, aoinit, aoloop and go
   SEE ALSO: aoread, aoinit, aoloop, go
 */
{
  aoread,parfile;
  aoinit,disp=disp,dpi=dpi,clean=clean,keepdmconfig=keepdmconfig;
  aoloop,disp=disp,dpi=dpi,controlscreen=controlscreen,savephase=savephase,savedm=savedm,saveslope=saveslope;
  after,0.1,go;
}
//----------------------------------------------------
...
*/
{
  ...
  for (i=1;i<=nwfs;i++) {
    ...
    if ( (wfs(i).type == "hartmann") || (wfs(i).type == "pyramid") ) {
      ...
      write,format = "npixpersub set to %i\n", int(npixpersub);
...
//---------------------------------------------------------------
func aoloop(disp=,savecb=,dpi=,controlscreen=,nographinit=,anim=,savephase=,no_reinit_wfs=,savedm=,saveslope=)
/* DOCUMENT aoloop(disp=,savecb=,dpi=,controlscreen=,nographinit=,anim=,savephase=,savedm=,saveslope=)
   ...
   savedm        = set to save dm voltages as fits file
   saveslope     = set to save measured slopes as fits file
...
//===========================
// Measurement of the turbulent phase
// Do the wavefront sensing
// Var: usedMes
// Func: mult_wfs (from yao_wfs.i)
//===========================
...
//===========================
// own crap
// save WFS measurement
//===========================  
  if (saveslope){
    fname = swrite(format="%s/%s_slopes_%04i.fits",YAO_SAVEPATH,parprefix,loopCounter);
    yao_fitswrite,fname,float(WfsMes),append=1,rescale=1;
  }
...
//==========================
  ...
  // err = dm values, converted from slopes
...
// Var: *dm(nm)._command (pointer to the mirror shape)
//===========================
  for (nm=1; nm<=ndm; nm++) {
...
//===========================
// own crap
// save actual dm shape
// *dm(nm)._command should be a pointer to the actual dm values
// This SHOULD be the ACTUAL dm shape, not only the converted slopes (needs proof)
//===========================
    if (savedm){
    fname = swrite(format="%s/%s_dm%01i_alt_%04i.fits",YAO_SAVEPATH,parprefix,nm,loopCounter);
    yao_fitswrite,fname,float(*dm(nm)._command);
    }
...
//===========================
// own crap
// Very simple OL function
// Just puts DM commands = 0
//===========================
  if (loop.method == "own-open-loop"){
    mircube *=0.0f;
  }
...
//===========================
// own crap
// save dm shape (how it really looks, spatially resolved)
//===========================
  if (savedm){
  fname = swrite(format="%s/%s_dm_%04i.fits",YAO_SAVEPATH,parprefix,loopCounter);
  yao_fitswrite,fname,float(mircube),append=0;
  }
  ...
  if (savephase||ok||okdisp) {
    ...
    phase_POL= get_phase2d_from_dms(residual_phase_which,residual_phase_what) - 
    ...
    if (savephase){
      fname = swrite(format="%s/%s_rwf_%04i.fits",YAO_SAVEPATH,parprefix,loopCounter);
      yao_fitswrite,fname,float(residual_phase*pupil),append=1;
    }

   // if (savephase){
   //   fname = swrite(format="%s/%s_rwfpol_%04i.fits",YAO_SAVEPATH,parprefix,loopCounter);
   //   yao_fitswrite,fname,float(phase_POL*pupil),append=1;
   // }
 ...
 */
{
  ...
  // Write the strehl into a file
  if (loop.method == "closed-loop"){
    g = open(YAO_SAVEPATH+parprefix+"_strehl","w");
    write,g,strehl(1,1);
    close,g;
  }
